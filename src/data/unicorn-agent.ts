...
import { logger as aiLogger, LogLevel } from '@/lib/logger';
...
export class UnicornAgent {
  ...
  private getHermesRecommendations(
    ...
    products = products.filter(p =>
      (p.priceCny ?? 0) >= prefs.priceRange.min &&
      (p.priceCny ?? 0) <= prefs.priceRange.max
    );
    ...
    products.forEach(product => {
      ...
      if (product.auctionData) {
        ...
        if (product.auctionData.soldPriceCny) {
          const priceDiff = product.auctionData.soldPriceCny - safePrice;
          const percentDiff = safePrice > 0 ? ((priceDiff / safePrice) * 100).toFixed(1) : '0.0';
  ...
  private calculateInvestmentRating(product: Product): 'excellent' | 'good' | 'fair' | 'poor' {
    ...
    if ((product.priceCny ?? 0) >= 500000) score += 25;
    else if ((product.priceCny ?? 0) >= 100000) score += 15;
  ...
  private generateSuggestions(
    ...
    // VIP suggestion
    if (userProfile.vipLevel === 'standard' && (product.priceCny ?? 0) >= 50000) {
