 ...
 */
export async function PUT(request: NextRequest): NextResponse<ApiResponse<any>> {
  try {
    const body = await request.json();
 ...
 */
export async function DELETE(request: NextRequest): NextResponse<ApiResponse<any>> {
  try {
    const { searchParams } = new URL(request.url);
    const cartId = searchParams.get('cartId');
